...
- (void)createCalendarViewWith:(NSDate *)date{
    ...
    headlabel.text     = [NSString stringWithFormat:@"%li-%li",[ZSCalenderTool year:date],[ZSCalenderTool month:date]];
    ...
    for (int i = 0; i < 42; i++) {
        ...
        NSInteger daysInLastMonth = [ZSCalenderTool totaldaysInMonth:[ZSCalenderTool lastMonth:date]];
        NSInteger daysInThisMonth = [ZSCalenderTool totaldaysInMonth:date];
        NSInteger firstWeekday    = [ZSCalenderTool firstWeekdayInThisMonth:date];
        ...
        // this month
        if ([ZSCalenderTool month:date] == [ZSCalenderTool month:[NSDate date]]) {
            
            NSInteger todayIndex = [ZSCalenderTool day:date] + firstWeekday - 1;
...
- (void)setSign:(int)i andBtn:(UIButton*)dayButton{
    [_signArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        int now = [dayButton.titleLabel.text intValue];
